...
bool Primos_Relativos(FactoresPrimos f1, FactoresPrimos f2)
{
    bool relativos = true;
    for (int i = 0; i < f1.TotalUtilizados(); i++)
    {
        ...
        {
            if(f1.Elemento(i)== f2.Elemento(j))
                relativos = false;
        }
    }
...
int main()
{
   ...
   if (numero1>1 && numero2>1) {
    ...
    cout << "\nDescomposicion del numero " << numero2 << endl;
      ...
      // FIXME: Usar la función para indicar si son primos relativos
    relativos = Primos_Relativos(f1, f2);
